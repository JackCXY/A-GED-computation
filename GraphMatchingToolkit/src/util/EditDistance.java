...
import java.util.TreeSet;//set集合的红黑树实现
...
public class EditDistance 
{
	...
	public double getEditDistance(Graph g1, Graph g2, CostFunction cf, int s, double bound) 
	{
		...
		TreeSet<TreeNode> open = new TreeSet<TreeNode>(); //open表
		// if the edges are undirected
		// all of the edge operations have to be multiplied by 0.5
		// since all edge operations are performed twice 
		double factor = 1.0;
...
//			System.out.println("*** OPEN: \n"+open);
			...
			TreeNode u = open.pollFirst(); //open表中头元素出队列
			if (u.allNodesUsed())
			{
				//u.printMatching();
				return u.getCost();
			...
			successors = u.generateSuccessors(bound);//在搜索树中怎么生成所有的后继结点，而且如何评价每个结点的代价
