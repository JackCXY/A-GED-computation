...
public class TreeNode implements Comparable<TreeNode> {
	...
	public TreeNode(Graph g1, Graph g2, CostFunction cf, double factor) {
		...
		this.inverseMatching = new int[g2.size()]; //the matching process
	...
	public LinkedList<TreeNode> generateSuccessors(double bound) {
		...
		{			
			TreeNode tn = new TreeNode(this); //copy constructors
			int n = tn.unusedNodes1.size();
			int e = 0;
			Iterator<Node> nodeIter = tn.unusedNodes1.iterator();//the node in Graph
			...
			tn.addCost(n * this.cf.getNodeCosts());//the left nodes in u1
			tn.addCost(e * this.cf.getEdgeCosts() * factor);//the left edges in u1
			tn.unusedNodes1.clear(); //free the space with u1 
		...
		{ // there are still nodes in g2 but no nodes in g1, the nodes of
			...
			{  
				//extend the next node in G1 and G2
				for (int i = 0; i < this.unusedNodes2.size(); i++)
				{
					TreeNode tn = new TreeNode(this);
					Node start = tn.unusedNodes1.remove();//remove the front elem in u1
					Node end = tn.unusedNodes2.remove(i); // remve the ith elem in u2
					tn.addCost(this.cf.getCost(start, end)); 
					//compuitng the cost between u1 and u2 
					...
					//get the matching process of G1 and G2 
				...
				tn.addCost(this.cf.getEdgeCosts() * e* factor);
				double c = (double)Math.round(tn.getCost() * 100000) / 100000; 
				if (c <= bound)
				{
					successors.add(tn);
...
//				}
	...
	private void processEdges(TreeNode tn, Node start, int startIndex, Node end, int endIndex) {
		...
		{
			if (tn.a1[startIndex][e] != null) 
			{ // there is an edge between start and start2
				Edge edge = tn.a1[startIndex][e];
				int start2Index = e;
				if (tn.matching[start2Index] != -1)
				{ // other end has been handled
					...
					{ // deletion the s2 in G1 
						tn.addCost(this.cf.getEdgeCosts() * factor);
					}
				}
			} 
			else 
			{ // there is no edge between start and start2
				int start2Index = e;
				if (tn.matching[start2Index] != -1) 
				{ // other "end" has been handled
					int end2Index = tn.matching[start2Index];
					if (end2Index >= 0) 
					{
						if (tn.a2[endIndex][end2Index] != null) //delete the edge
						{
							tn.addCost(this.cf.getEdgeCosts()* factor);
