...
public class EditDistance {
	...
	public double getEditDistance(Graph g1, Graph g2, CostFunction cf, int s, double bound) {
		...
		open.add(start);//add the start node into List(map)
		...
		while (!open.isEmpty()){	
			eTime = System.currentTimeMillis(); //the computing time can not exceed 30s
			if (eTime-sTime > MAX_TIME){
				return -1;
			}
			TreeNode u = open.pollFirst();  //First node in List (according to the order)
			if (u.allNodesUsed()){
				return u.getCost();
			}
			...
			// is limited to s
			while (open.size() > s){ //s can be bigger
				open.pollLast();
			}		
		}
		// error case 
